...
- 画像から続きを描いて動画は保存する設定の時に、何も描画せずに投稿を完了すると、配列が空の不正な動画データが作成されていました。  
そのため続きを描いた時に動画データから画像をキャンバスに読み込む事ができなくなっていました。  
この問題に対応するため、描画履歴の配列が空の時は、アンドゥの履歴をプッシュして空の配列にならないようにしました。  
